...
class python_imager(object):
    def __init__(self,npix,nsource,max_I, #Params for sky model
        ...
        ra0 = ref_ra*pi/180.0
        dec0 = ref_dec*pi/180.0
        
        ra = ra0 + linspace(ra0 - npix*delta_pix/2.0,ra0 + npix*delta_pix/2.0,npix)
        ra_PSF  = ra0 + linspace(ra0 - npix*delta_pix,ra0 + npix*delta_pix,2*npix)
        delta_ra = ra - ra0
        delta_ra_PSF = ra_PSF - ra0
        dec = dec0 + linspace(dec0 - npix*delta_pix/2.0,dec0 + npix*delta_pix/2.0,npix)
        dec_PSF = dec0 + linspace(dec0 - npix*delta_pix,dec0 + npix*delta_pix,2*npix)
        
        #Get corresponding l and m
        l = (cos(dec)*sin(delta_ra))
        l_PSF = (cos(dec_PSF)*sin(delta_ra_PSF))
        m = (-sin(dec)*cos(dec0) - cos(dec)*sin(dec0)*cos(delta_ra))
        m_PSF = (-sin(dec_PSF)*cos(dec0) - cos(dec_PSF)*sin(dec0)*cos(delta_ra_PSF))

        print "Getting lmn"
        lmn = self.get_lmn(l,m)
        self.lmn = lmn
        ncoord = lmn[:,2] + 1.0
        lmn_PSF = self.get_lmn(l_PSF,m_PSF)
        ncoord_PSF = lmn_PSF[:,2] + 1.0
        ...
        if mode=='Full':
            ...
            #Get DFT kernels to compute PSF and ID
            print "Getting DFT kernels for PSF"
            ref_lambda = 0.25
            K_PSF = self.set_kernels(lmn_PSF,ref_lambda)
            invK_PSF = K_PSF.conjugate().T

            print "Computing PSF"
            PSF = self.iDFT(ones(self.nvis),invK_PSF,ncoord_PSF)
            PSF = abs(PSF.reshape(2*npix,2*npix))
            print PSF.max()
            ...
            K_DFT = self.set_kernels(lmn,ref_lambda)
            invK_DFT = K_DFT.conjugate().T            
            
            #Use DFT to simulate data + noise 
            #Get model vis
            self.vis_uv = self.DFT(self.model_sky.copy().flatten(),K_DFT,ncoord)
            #Add some noise
            sigma = 5.0
            self.svisdat = ones(self.nvis)*(sigma*self.vis_uv + 1j*sigma*self.vis_uv)
            self.svisdatsq = real(self.svisdat*self.svisdat.conj()) + 1.0e-10 #precompute to speed up likelihood evaluation
            self.visdat = self.vis_uv + sigma*self.vis_uv*random.randn(self.nvis) + 1j*sigma*self.vis_uv*random.randn(self.nvis)            
            
            #Make dirty image from noisy data
            print "Getting ID"
            self.ID = real(self.iDFT(self.visdat,invK_DFT,ncoord)).reshape(npix,npix)
            
            plt.imshow(self.ID,interpolation="nearest", cmap="cubehelix")
            plt.colorbar()            
...
#        self.l0 = cos(reference_dec_rad)*sin()
    ...
    def get_lmn(self,l,m):
        ...
        n = zeros(Nx*Ny) #array to store
        lmn = zeros([Nx*Ny,3]) #array to store combos of l, m and n
        for i in range(Nx):
            for j in range(Ny):
                n[i*Ny + j] = sqrt(1.0 - l[i]**2 - m[j]**2)
                lmn[i*Ny + j,0] = l[i]
                lmn[i*Ny + j,1] = m[j]
                lmn[i*Ny + j,2] = n[i*Ny+j] - 1
        return lmn
    
    def set_kernels(self,lmn,ref_lambda):
        """
        Sets the DFT kernel
        """
        return exp(-2.0j*pi*dot(self.uvw,lmn.T)/ref_lambda)
    
    def DFT(self,I,K,n):
        """"
        Returns DFT of image I
        """
        return dot(K,I/n)
    
    def iDFT(self,vis,invK,n):
        """
        Returns iDFT (to grid and compute PSF)
        """
        return n*dot(invK,vis)
...
if __name__ == "__main__":
    ...
    ref_dec = 0.0 #In degrees
    observation_length_in_hrs = 0.25
